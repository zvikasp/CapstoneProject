	...
	<table width="60%" cellspacing="20">
		<tr bgcolor="#75c7e6">
			...
			try {
				...
				ResultSet rs2 = null;
				while (rs.next()) {
					rs2 = st2.executeQuery("select tid_ct, tname_ct, screen_ct from avail_tab where tid_ct='" + rs.getString(1) 
					...
					while (rs2.next()) {
						...
						<tr style="border: 1px solid; background-color:#befaaf;">
							...
							<td>
								<h4>
									<a href="screenseats2?1=<%=rs2.getString(1)%>&&2=<%=rs2.getString(2)%>&&3=<%=rs2.getString(3)%>">
									<%=rs2.getString(3)%></a>
								</h4>
							</td>							
						</tr>
